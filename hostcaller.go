...
// cgoHostCaller implements bridge.HostCaller over the C-ABI host callbacks stored on
...
func (cgoHostCaller) Call(method string, req []byte) ([]byte, error) {
	...
	mu.Unlock()
	if call == nil {
		return nil, errors.New("mcp-bridge: host not connected")
	}
	...
	if resp == nil {
		...
		return nil, errors.New("mcp-bridge: host call failed")
	}
